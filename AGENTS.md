...
Kimera visualization in this ROS wrapper has two separate switches that are
easy to confuse: `kimera_visualize` keeps Kimera's visualization pipeline alive,
and `use_kimera_rviz` selects the ROS/RViz visualizer backend instead of the
OpenCV/VTK backend. Docker Rerun-only runs should keep
`kimera_visualize:=true` and `use_kimera_rviz:=true`, while leaving
`use_liorf_rviz:=false`. Do not disable `kimera_visualize` or set
`use_kimera_rviz:=false` just to avoid a display crash, because that either
stops Kimera from feeding `RosVisualizer` or falls back to the OpenCV/VTK
windowing path.
